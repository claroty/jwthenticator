...
class TokenManager:
    ...
    def __init__(
        ...
        jwt_lease_time: int = JWT_LEASE_TIME,
        jwt_audience: List[str] = JWT_AUDIENCE,
    ...
    async def create_access_token(self, identifier: UUID) -> str:
        ...
        token_string = jwt.encode(
            encoded_payload, self.private_key, self.algorithm, headers=self.jwt_headers
        )
        return token_string
    ...
    async def create_refresh_token(
        self, key_id: int, expires_at: Optional[datetime] = None
    ) -> str:
        """
        ...
        if expires_at is None:
            expires_at = expires_at = datetime.utcnow() + timedelta(
        ...
        async with self.session_factory() as session:
            refresh_token_info_obj = RefreshTokenInfo(
                expires_at=expires_at, token=refresh_token_str, key_id=key_id
            )
    ...
    async def check_refresh_token_exists(self, refresh_token: str) -> bool:
        ...
        async with self.session_factory() as session:
            if (
                await session.query(RefreshTokenInfo)
    ...
    async def load_refresh_token(self, refresh_token: str) -> RefreshTokenData:
        ...
        async with self.session_factory() as session:
            refresh_token_info_obj = (
                await session.query(RefreshTokenInfo)
                .filter_by(token=refresh_token)
                .first()
            )
            refresh_token_data_obj = self.refresh_token_schema.load(
                self.refresh_token_schema.dump(refresh_token_info_obj)
            )
